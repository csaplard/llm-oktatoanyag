(function () {
  ...
  function mount(spec) {
    ...
    card.innerHTML = '<figcaption><span class="lab-kicker">' + t({hu:'Interaktív műhely',en:'Interactive workshop'}) + '</span><h4 id="' + card.id + '-title">' + esc(t(spec.title)) + '</h4><p>' + esc(t(spec.intro)) + '</p></figcaption><div class="lab-controls"></div><div class="lab-result" aria-live="off"></div><p class="lab-challenge"><strong>' + t({hu:'Próbáld ki: ',en:'Try it: '}) + '</strong>' + esc(t(spec.challenge)) + '</p><details class="lab-boundary"><summary>' + t({hu:'Mit mutat ez a példa?',en:'What does this example show?'}) + '</summary><p>' + esc(t(spec.limitation)) + '</p></details><div class="lab-actions"><button type="button" class="lab-reset">' + t({hu:'Alaphelyzet',en:'Reset example'}) + '</button><a href="#lab-directory">' + t({hu:'Összes műhely ↑',en:'All workshops ↑'}) + '</a></div>';
  ...
  function addReadingControls() {
    ...
    [{id:'alapok',hu:'Alapozó segítség',en:'Foundation help'},{id:'lab-directory',hu:'Interaktív műhelyek',en:'Interactive workshops'},{id:'tanulasi-ut',hu:'12 hetes útvonal',en:'12-week path'}].forEach(function (entry) {
